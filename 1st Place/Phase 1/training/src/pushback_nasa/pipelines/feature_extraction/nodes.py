...
def compute_perimeter(
    labels: pd.DataFrame, sub_format: pd.DataFrame, airport: str
) -> pd.DataFrame:
    sub_format = sub_format[sub_format["airport"] == airport]
    perimeter = (
        pd.concat([labels, sub_format])
...
def extract_tfm_features(tfm: pd.DataFrame) -> pd.DataFrame:
    # Stick only with records where estimated arrival is larger than timestamp
    tfm = tfm[tfm["timestamp"] <= tfm["arrival_runway_estimated_time"]]

    # Stick only with records where the estimated arrival is in the next 3h
    tfm = tfm[
        tfm["timestamp"] >= tfm["arrival_runway_estimated_time"] - pd.Timedelta("3h")
    ]

    # Sort by gufi and timestamp
    tfm = tfm.sort_values(["gufi", "timestamp"])

    # Create tbfm delay variable as the difference in shifted runway estimated arrival
    tfm.loc[
        tfm["arrival_runway_estimated_time"]
        > tfm["arrival_runway_estimated_time"].shift(),
        "tfm_delay",
    ] = (
        (
            tfm["arrival_runway_estimated_time"]
    ...
    tfm.loc[tfm["gufi"] != tfm["gufi"].shift(), "tfm_delay"] = 0

    # Ceil timestamp to 15 minute periods
    tfm["timestamp"] = tfm["timestamp"].dt.ceil("15min")

    # Compute features aggregated at the timestamp level
    tfm_features = (
        tfm.groupby("timestamp")
        .agg(
            tfm_n_gufis=("gufi", "nunique"),
            tfm_tot_delay=("tfm_delay", "sum"),
            tfm_max_delay=("tfm_delay", "max"),
            tfm_mean_delay=("tfm_delay", "mean"),
        )
        .reset_index()
    )
    tfm_features["tfm_delay_per_gufi"] = (
        tfm_features["tfm_tot_delay"] / tfm_features["tfm_n_gufis"]
    )

    return tfm_features


def extract_tbfm_features(tbfm: pd.DataFrame) -> pd.DataFrame:
    # Stick only with records where estimated arrival is larger than timestamp
    tbfm = tbfm[tbfm["timestamp"] <= tbfm["scheduled_runway_estimated_time"]]

    # Stick only with records where the estimated arrival is in the next 3h
    tbfm = tbfm[
        tbfm["timestamp"]
        >= tbfm["scheduled_runway_estimated_time"] - pd.Timedelta("3h")
    ]

    # Sort by gufi and timestamp
    tbfm = tbfm.sort_values(["gufi", "timestamp"])

    # Create tbfm delay variable as the difference in shifted runway estimated arrival
    tbfm.loc[
        tbfm["scheduled_runway_estimated_time"]
        > tbfm["scheduled_runway_estimated_time"].shift(),
        "tbfm_delay",
    ] = (
        (
            tbfm["scheduled_runway_estimated_time"]
    ...
    tbfm.loc[tbfm["gufi"] != tbfm["gufi"].shift(), "tbfm_delay"] = 0

    # Ceil timestamp to 15 minute periods
    tbfm["timestamp"] = tbfm["timestamp"].dt.ceil("15min")

    # Compute features aggregated at the timestamp level
    tbfm_features = (
        tbfm.groupby("timestamp")
        .agg(
            tbfm_n_gufis=("gufi", "nunique"),
            tbfm_tot_delay=("tbfm_delay", "sum"),
            tbfm_max_delay=("tbfm_delay", "max"),
            tbfm_mean_delay=("tbfm_delay", "mean"),
        )
        .reset_index()
    )
    tbfm_features["tbfm_delay_per_gufi"] = (
        tbfm_features["tbfm_tot_delay"] / tbfm_features["tbfm_n_gufis"]
    )

    return tbfm_features
