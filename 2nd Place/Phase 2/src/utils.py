...
def _load_features(airport: str, feature_name: str) -> pd.DataFrame:
    if feature_name in FEATURES[airport]:
        ...
        FEATURES[airport][feature_name] = pd.read_csv(
            data_directory / "public" / airport / f"{airport}_{feature_name}.csv.bz2",
...
def _get_censored_features(
    ...
    for feature_name in [
        ...
        if feature_name == "mfs" or feature_name == "standtimes":  # private feature
            censored_features[feature_name] = None
            continue
    ...
    mfs_data = pd.concat(mfs_data, axis=0)

    censored_features["mfs"] = mfs_data.loc[mfs_data.gufi.isin(gufis)]
...
def compute_predictions():
    ...
    for prediction_time in _log_progress(
        ...
        for airport in airports:
            ...
            valid_time_str = str(prediction_time).replace(" ", "-").replace(":", "_")
            partial_prediction_path = (
                prediction_directory / f"{airport}_{valid_time_str}.csv"
            )
